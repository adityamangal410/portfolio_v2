...
Looks like few of the most popular Artists in this dataset are Britney Spears, Lady Gaga and Christina Aguilera and as expected, Rock and Pop are the most popular tags. Lets see which are the most popular tags that are assigned to the artists by the users.

```{r}
metadata %>% 
  filter(!is.na(artistName)) %>% 
  count(artistName, tagValue, sort = TRUE) %>% 
  top_n(20, n)
```
