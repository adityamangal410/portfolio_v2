...
recipe(target ~ ., data = tweets_train) %>% 
  ...
  step_normalize(all_numeric(), -all_outcomes()) -> tweets_recipe 
  # step_pca(all_predictors(), num_comp = 2)-> tweets_recipe
...
Let's try and tune the regularization penalty and visualize the results. 

#### Tuning lasso penalty

Using 10-fold cross validation and values of K going from 1 to 100. 
...
Above plot again shows that we are dealing with a less flexible problem space.
...
The f1-score metric comes out to be 0.81 which is much higher compared to the baseline and also the K-Nearest Neighbor model that we built in [Part 2](/2020/02/nlp-with-disaster-tweets-part-2-nearest-neighbor-models/){target="_blank"}. Lets try and visualize the variable importance for different features that we used to train our model. 
...
lasso_last_fit$.workflow[[1]] %>% 
  pull_workflow_fit() %>% 
  tidy() %>% 
  filter(term %in% c("len", "num_hashtags", "v288", "v472")) %>% 
  select(term, estimate, lambda) %>% 
  pivot_wider(names_from = term, values_from = estimate) %>% 
  replace_na(list(num_hashtags = 0, v472 = 0, len = 0, v288 = 0)) %>% 
...
In the next part of this series, I will start building more non-parametric models like tree-based models and see if we can get better scoring models. 
...
# References
 ...
 - Lasso Regression using [Tidymodels](https://juliasilge.com/blog/lasso-the-office/) by Julia Silge.