...
The above shows a simple K-nearest neighbors model using the "kknn" engine. Gives about 0.34 of minimal misclassification. 
...
knn_grid %>% 
  ...
  labs(title = "Model performance against model flexibility",
       x = "Model flexibility (1/NumberOfNeighbors)",
       y = "Mean metric value")
```
...
Our final fit knn model with K=20 gives an f1-score of 0.7610, which is much higher than our baseline model on the same CV dataset.   
